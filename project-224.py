...
def Bike_control():
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.54))
    time.sleep(15)
    dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
    time.sleep(5)
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.25, steer=-0.36))
    ...
    time.sleep(2)
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.30))
    dropped_vehicle.set_light_state(
        carla.VehicleLightState(
            carla.VehicleLightState.HighBeam | carla.VehicleLightState.Brake
        )
    )
    time.sleep(3)
    dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        ...
        for i in object_id:
            if (
                distance_name_data["name"] == "Buildings"
                and distance_name_data["distance"] > 3
                and distance_name_data["distance"] < 12
            ):
                dropped_vehicle.set_light_state(
                    carla.VehicleLightState(carla.VehicleLightState.NONE)
                )
                ...
                print(
                    f"Object near by our car: {i} and the distance is: {distance_name_data['distance']}"
                )
...
try:
    ...
    dropped_vehicle.set_light_state(
        carla.VehicleLightState(carla.VehicleLightState.HighBeam)
    )
    ...
    lidar_sensor_data = world.spawn_actor(
