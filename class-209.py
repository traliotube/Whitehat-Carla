...
try:
    ...
    def _on_collision(data):
        print("Collision!")
        print(f"Collision with {getActorName(data.other_actor)}")
        collisionEventRecord = data.normal_impulse
        intensity = math.sqrt(collisionEventRecord.x ** 2 + collisionEventRecord.y ** 2 + collisionEventRecord.z ** 2)
        print(f"Collision intensity: {intensity}")
        dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
        time.sleep(5)
