...
def number_of_vehicle():
    ...
    if len(all_vehicles) > 1:
        def distance(data): return math.sqrt(
        ...
        for each_car in all_vehicles:
            if each_car.id != world.id:
                get_distance_of_bot_vehicles.append(
                    (distance(each_car.get_location()), each_car))
        ...
        for distance_of_car, vehicle in sorted_vehicles:
            ...
            for distance_in_meter in final_vehicle_result:
                if distance_in_meter['distance'] > 6 and distance_in_meter['distance'] < 10:
                    ...
                    time.sleep(2)
                    dropped_vehicle.apply_control(
                        carla.VehicleControl(throttle=0.2, steer=0.1))
                    time.sleep(2)
                    dropped_vehicle.apply_control(
                        carla.VehicleControl(throttle=0.2))
                    time.sleep(3)
...
def car_control():
    ...
    dropped_vehicle.set_light_state(
        carla.VehicleLightState(carla.VehicleLightState.All))
    time.sleep(5)
...
try:
    ...
    car_model = get_blueprint_of_world.filter(
    ...
    sensor_camera_spawn_point = carla.Transform(
        carla.Location(x=-11.5, z=4.7, y=1.2))
