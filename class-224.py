...
def generate_lidar_blueprint(blueprint_library):
    lidar_blueprint = blueprint_library.find(
        "sensor.lidar.ray_cast_semantic"
    )  # write code here for lidar
    lidar_blueprint.set_attribute("channels", str(64))  # write code here
    lidar_blueprint.set_attribute("points_per_second", str(56000))  # write code here
...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        ...
        distance_name_data["name"] = key_list[detection.object_tag]
        for object_name in object_id:
            if (
                distance_name_data["name"] == object_name
                and distance_name_data["distance"] > 5
                and distance_name_data["distance"] < 7
            ):
                dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.31))
                if (
                    distance_name_data["distance"] > 3
                    and distance_name_data["distance"] < 6
                ):
                    dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
                    ...
                    print(
                        distance_name_data["name"], ": ", distance_name_data["distance"]
                    )
                    break
            else:
                print(distance_name_data["name"], ": ", distance_name_data["distance"])
