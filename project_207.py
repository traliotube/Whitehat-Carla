...
def car_control():
    ...
    time.sleep(6)
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.5, steer=-0.6))
    time.sleep(4)
...
try:
    ...
    car_model = random.choice(get_blueprint_of_world.filter(‘vehicle.*’)[0])
    ...
    if car_new_location != Location(x=66.648193, y=0.766224, z=0.162060):
        dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.5, reverse=True))
        time.sleep(5)
    else:
        dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.5))
        time.sleep(5)
