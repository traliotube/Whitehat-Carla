...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        steerIncrement = 0.0005 * milliseconds
        if keys[K_LEFT] or keys[K_a]:
            if self._steerCache > 0:
                self._steerCache = 0
            else:
                self._steerCache = self._steerCache - steerIncrement
        ...
        self._steerCache = min(0.7, max(-0.7, self._steerCache))
        self._control.steer = round(self._steerCache, 1)
        driving_data["steer"] = self._control.steer

        store_driving_data.append(driving_data)

        field_names = ["brake", "hand_brake", "throttle", "steer"]
        csvfilestore = open("manual_driving_data.csv", "w", newline="")
